 ...
 * Wenn der .more-options-item-list-view Button geklickt wird hat der Nutzer die möglichkeit den jeweiligen Eintrag aus der Liste zu entfernen.
 *
...
playlist.addEventListener('click', function (event) {
  ...
  if (selectedListElement !== null) {
    ...
    if (clickedElement.classList.contains('more-options-item-list-view')) {
      let result = confirm(
        'Willst du wirklich ' + title + ' von ' + url + ' löschen?'
      );
      if (result) {
        playlist.removeChild(selectedListElement);
      }
    } else {
      alert(title);
    }
